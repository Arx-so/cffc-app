...
const Login = () => {
  const { email, password, setEmail, setPassword, onLoginPress, isLoading } = useLogin();
    ...
    <KeyboardAvoidingView style={S.keyboardAvoid} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <Stack.Screen
        options={{
          ...
          headerTitleStyle: { color: Brand.white, fontWeight: 'bold', fontSize: 14 },
          headerBackTitle: '',
        }}
      />
      ...
      >
        <Text style={[S.heroSubtitle, { marginBottom: 6 }]}>BEM-VINDO DE VOLTA</Text>
        ...
        >
          {googleLoading
            ...
            : (
              <>
                <Ionicons name="logo-google" size={18} color={Brand.white} style={{ marginRight: 10 }} />
        ...
        <TouchableOpacity style={S.bottomLink} onPress={() => router.push('/signup')} disabled={busy}>
